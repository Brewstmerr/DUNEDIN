...
(function () {
  ...
  app.controller('prmLoginAlmaMashupAfterController', ['$element', '$scope', '$location', function ($element, $scope, $location) {
    ...
    $scope.frvSignIn = function () {
      var signin_choice = document.getElementById("frv_signin_choice");
  ...
  app.controller('userWarning', function (getUserFines, getUserLoans, getUserRequests, getUserMsgs, $scope) {
    ...
    $scope.hasMsgs = false;
    if ($scope.$parent.$parent.$ctrl.userName() != "") {
      //only request fines if the user is logged in
      //fines warning
      try {
  ...
  app.controller('signout', function (getSignoutInfo, $scope, $http) {
    ...
    if ($scope.$parent.$parent.$ctrl.userName() != "" && typeof $scope.$parent.$parent.$ctrl.userName() != "undefined") {
      //only request fines if the user is logged in
      try {
        ...
        getSignoutInfo.async().then(function (response) {
          ...
          if (shibCodes.includes(patronCode)) {
            ...
            $http.get('https://api.cognitive.microsoft.com/bing/v7.0/search').then(function (response) {
              ...
              //error: as expected
              if (data.status == -1) {
                //status is 401 if not going through zen
                // behind zen
                $scope.notShib = false;
                $scope.shib = true;
              }
  ...
  app.controller('prmAccountOverviewAfterController', ['$element', '$scope', '$compile', '$http', function ($element, $scope, $compile, $http) {
    ...
    $http.get('/primo_library/libweb/webservices/rest/v1/myaccount/fines').then(function (response) {
      try {
        var finesLength = response.data.data.fines.fine.length;
      } catch (e) {}
      if (finesLength > 0) {
              ...
              var advancedSearchObs2 = new MutationObserver(function (mutations2) {
                mutations2.forEach(function (mutation2) {
                  ...
                  for (var i = 0; i < mutation2.addedNodes.length; i++) {
                    ...
                    if (node.nodeName == "PRM-SEARCH-RESULT-SORT-BY") {
                      console.log("results there");
  ...
  app.controller('prmSearchResultListAfterController', ['$scope', '$compile', function ($scope, $compile) {
    ...
    if (window.location.href.indexOf('primo-explore/jsearch?') > 1 && window.location.href.indexOf('&journals=letter,') > 1) {
      showSort = false;
    }
    if (showSort == true) {
  ...
  app.controller('prmBriefResultAfterController', ['$http', '$element', '$scope', '$compile', function ($http, $element, $scope, $compile) {
    ...
    ctrl.createDirectLink = function () {
      ...
      if ($scope.$parent.$ctrl.item.pnx.delivery.delcategory[0] != 'Alma-P' && $scope.$parent.$ctrl.resourceTypeForDisplay != 'mediatype.multiplever') {
        var directLinks = function directLinks() {
          if (typeof $scope.$parent.$ctrl.item.delivery != "undefined") {
            var title_dl = angular.element(container);
  ...
  app.controller('prmSearchResultAvailabilityLineAfterController', ['$element', '$scope', '$compile', '$http', function ($element, $scope, $compile, $http) {
    ...
    ctrl.createDirectLink = function () {
      ...
      if (delCat == "Alma-P") {
        var cenFloorInfo = function cenFloorInfo() {
          if (typeof $scope.$parent.$ctrl.libraryCode != "undefined") {
            var _cenBooksFloorLoop = function _cenBooksFloorLoop() {
      ...
      if (ctrl.parentCtrl.item.delivery.GetIt1[0].category != "Alma-P") {
        ...
        Primo.records.then(function () {
          if (ctrl.parentCtrl.$document[0].URL.indexOf("primo-explore/openurl?") > 0) {
            ...
            if (window.location.href.indexOf('ezproxy.otago.ac.nz') >= 0) {
              //if users are already auth'ed against ezproxy
              urlStart = 'www.otago.ac.nz.ezproxy.otago.ac.nz/library/primo/viewresource.php?resource=';
            } else if (window.location.href.indexOf('ereserve.otago.ac.nz') >= 0) {
              //if users are already auth'ed against ereserve ezproxy
              urlStart = 'www.otago.ac.nz.ereserve.otago.ac.nz/library/primo/viewresource.php?resource=';
            } else if (window.location.href.indexOf('alproxy.otago.ac.nz') >= 0) {
              //if users are already auth'ed against test ezproxy
            ...
            var attachPoint = document.getElementsByClassName("section-body")[0];
  ...
  app.controller('prmFinesOverviewAfterController', ['$element', '$scope', function ($element, $scope) {
    var ctrl = this;
    ...
    ctrl.insertPayNowBtn = function () {
      var parentElement = ctrl.$element.parent();
      var container = angular.element(parentElement.children()[0].children[0]);
      try {
        document.getElementById("pay_fines_btn_overview").style.display = 'none';
      } catch (e) {
        console.log(e);
      }
      // container[0].children[0].children[0].append(ctrl.$element.children()[0]);
      ...
      Primo.user.then(function (user) {
        if (user.fines.length > 0) {
          try {
            document.getElementById("pay_fines_btn_overview").style.display = 'block';
          } catch (e) {
            console.log(e);
          }
...
(function () {
  ...
  function checkStatus() {
    ...
    if (null === chat_load.querySelector("iframe")) if (qs = "https://" + libchat_options.base_domain + "/chati.php?", qs += "hash=" + libchat_options.hash, window.document.title && (qs += "&referer_title=" + encodeURIComponent(window.document.title)), !1 !== online || !0 !== chat_self_triggered) {
      !0 === chat_self_triggered && (chat_self_triggered = !1, qs += "&auto=true", chat_load.setAttribute("aria-live", "polite"), chat_load.setAttribute("aria-hidden", "false")), window.addEventListener("message", function (e) {
        var data = e.data;"object" == (typeof data === 'undefined' ? 'undefined' : _typeof(data)) && data.action && ("closeWidget" === data.action ? (chat_div.className = chat_div.className.replace(/(^|\s)open(\s|$)/, ""), chat_load.removeAttribute("aria-live"), chat_load.setAttribute("aria-hidden", "true")) : "chatStarted" === data.action || "autopop" === data.action && -1 === chat_div.className.indexOf("open") && (chat_div.className += " open"));
    ...
    function load(url, pfnError) {
      var script = document.createElement("script"),
          done = !1;script.src = url, script.async = !0;var errorHandler = pfnError || config.error;"function" == typeof errorHandler && (script.onerror = function (ex) {
      ...
      var key,
          query = -1 === (url || "").indexOf("?") ? "?" : "&",
          uniqueName = (callbackName = callbackName || config.callbackName || "callback") + "_json" + libchat_options.hash;params = params || {};for (key in params) {
        params.hasOwnProperty(key) && (query += encode(key) + "=" + encode(params[key]) + "&");
      }return window[uniqueName] = function (data) {
        callback(data);try {
          delete window[uniqueName];
        } catch (e) {}window[uniqueName] = null;
      }, load(url + query + callbackName + "=" + uniqueName), uniqueName;
    }function setDefaults(obj) {
      config = obj;
    }var head,
        window = this,
        config = {};return { get: jsonp, init: setDefaults };
