...
/*app.component('prmUserAreaAfter', {
	...
	controller: function($compile, $scope, $templateCache, $element) {
		...
		<div layout="column">\
			<div id="user_name" class="user-name">{{$ctrl.userName()}}</div>\
			<prm-library-card-menu id="library_card_button"></prm-library-card-menu>\
			<prm-authentication layout="flex" [is-logged-in]="$ctrl.userName().length > 0"></prm-authentication>\
			<div id="user_warnings" ng-controller="userWarning">\
				<div class="usermenu_warn" ng-show="hasFines">\
					<div id="fines_warning_icon_div">\
						<md-icon id="fine_warning" icon-type="svg" md-svg-src="/primo-explore/custom/DUNEDIN/img/fines_warn.svg" class="md-primoExplore-theme"></md-icon>\
					</div>\
				</div>\
				<div class="usermenu_warn" ng-show="hasOverdues">\
					<div id="overdue_warning_white_background"></div>\
					<prm-icon id="overdue_warning" class="prm-warn" icon-definition="timer" icon-type="svg" svg-icon-set="primo-ui"></prm-icon>\
				</div>\
				<div class="usermenu_warn" ng-show="hasRequests">\
					<img id="request_background" src="/primo-explore/custom/DUNEDIN/img/bell_background.png"></img>\
					<prm-icon id="request_warning" class="prm-positive" icon-definition="bell" icon-type="svg" svg-icon-set="primo-ui"></prm-icon>\
				</div>\
				<div class="usermenu_warn" ng-show="hasMsgs">\
					<prm-icon id="msg_warning" class="prm-notice" icon-definition="flag" icon-type="svg" svg-icon-set="primo-ui"></prm-icon>\
				</div>\
			</div>\
			<button id="mobile_menu" class="mobile-menu-button zero-margin md-button md-primoExplore-theme md-ink-ripple hide-gt-xs" type="button" aria-label="Open user actions menu button" (click)="$ctrl.enableMobileMenu()" style="min-width: 60px" hide-gt-xs>\
				<prm-icon [icon-type]="::$ctrl.topBarIcons.more.type" [svg-icon-set]="::$ctrl.topBarIcons.more.iconSet" [icon-definition]="::$ctrl.topBarIcons.more.icon">\
					<md-icon md-svg-icon="primo-ui:dots-horizontal" aria-label="icon-dots-horizontal" class="md-primoExplore-theme" aria-hidden="true">\
						<svg id="dots-horizontal_cache305" width="100%" height="100%" viewBox="0 0 24 24" y="120" xmlns="http://www.w3.org/2000/svg" fit preserveAspectRatio="xMidYMid meet" focusable="false">\
...
app.controller('prmSearchBarAfterController', function($scope) {
	...
	// attHide.value = "document.getElementById('prm-simple-search').style.display = 'none';";
	attHide.value = "document.getElementById('prm-simple-search').style.display = 'none' & document.querySelector('.search-actions.layout-align-xs-start-center [aria-label=\"Search\"]').focus()";
...
app.controller('prmBriefResultAfterController', ['$element', '$scope', '$compile', function($element, $scope, $compile) {
	...
	ctrl.createDirectLink = function() {
		let parentElement = ctrl.$element.parent();
		let container = angular.element(parentElement.children()[0]);
		//All E stuff needs a direct link if it's nor frbr
		if (($scope.$parent.$ctrl.item.pnx.delivery.delcategory[0] != 'Alma-P' && $scope.$parent.$ctrl.resourceTypeForDisplay != 'mediatype.multiplever')) {
			...
			//let title_dl = angular.element(container.children()[0]);
			...
			if (window.location.href.indexOf("primo-explore/fulldisplay?") == -1) {
				...
				// viewDetails_div.setAttribute("class", "layout-align-start-start layout-row");
				// viewDetails_div.setAttribute("layout", "row");
...
app.controller('prmFinesAfterController', ['$element', '$scope', '$compile', '$http', function($element, $scope, $compile, $http) {
	...
	ctrl.insertPayNowBtn = function() {
		let parentElement = ctrl.$element.parent();
		let container = angular.element(parentElement.children()[0].children[0]);
		document.getElementById("pay_fines_btn_main").style.display = 'none';
		// container[0].children[0].append(ctrl.$element.children()[0]);
		...
		$http.get('/primo_library/libweb/webservices/rest/v1/myaccount/fines').then(function(user) {
			if (user.fines.length > 0) {
				document.getElementById("pay_fines_btn_main").style.display = 'block';
				$scope.payFines = function () {
					//open window now to avoid pop-up blockers
					window.open('', 'pay_fines_window');
					//after username returned, set the hidden element in the "open fines page" form
					var fobj = document.getElementById('pay_fines_uname_main');
...
ga('create', 'UA-36296579-1', 'auto');
ga('send', 'pageview');

// University of Otago Global
ga('create', 'UA-9031494-1', 'auto', {'name': 'b'});
ga('b.send', 'pageview');*/
